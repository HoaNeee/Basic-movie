...
const DetailPage = () => {
  ...
  const { data: dataDetails, loading: loadingDetails } = useFetchDetail(
    `/${params.explore}/${params.id}`
  );

  const { data: credits, loading: loadingCredits } = useFetchDetail(
    `/${params.explore}/${params.id}/credits`
  );

  const { data: similarData, loading: loadingSimilars } = useFetch(
    `/${params.explore}/${params.id}/similar`
  );

  const { data: recommendations, loading: loadingRecomends } = useFetch(
    `/${params.explore}/${params.id}/recommendations`
  );

  const { data: videosData, loading: loadingVideos } = useFetch(
            ...
            <div className="container px-2 relative flex mx-auto lg:flex-row flex-col lg:gap-10 gap-5 items-center lg:items-start">
              ...
              <div className="flex-1">
                <div className="text-center lg:text-start">
                  ...
                  <div
                    className="inline-flex items-center text-black gap-2 my-4 px-4 rounded bg-white md:bg-gradient-to-r from-orange-600 to-yellow-400 group cursor-pointer transition-all hover:scale-105"
                    onClick={handleTrailer}
                  >
                    <FaPlay className="cursor-pointer" />
                    <button className="font-bold py-2 cursor-pointer">
                      {language === "vi" ? "Giới thiệu" : "Trailer"}
                    </button>
                  </div>
                  <p className="text-neutral-400 ml-1 ">
                ...
                <div className="flex gap-3 items-center">
                  ...
                  <p>
                    ...
                    {Number(dataDetails.popularity || 0).toFixed(1)}
                  </p>
                  <span>|</span>
                  <p>
                    {language === "vi" ? "Thời lượng" : "Duration"} :{" "}
                    {Number(dataDetails.runtime / 60 || 0).toFixed(0)}
                    {language === "vi" ? " giờ " : "h"}
                    {Number(dataDetails.runtime % 60 || 0).toFixed(0)}
                    {language === "vi" ? " phút" : "m"}
                  </p>
                ...
                <div>
                  <h2 className="mb-3 text-white text-lg font-bold">
                    {language === "vi" ? "Các diễn viên: " : "Star Cast :"}{" "}
                  </h2>
            ...
            <div className="mt-10 flex flex-col gap-3">
              {similarData && similarData.length > 0 && (
                <MovieList
                  ...
                  movies={similarData}
                  title={`${language === "vi" ? "Tương tự" : "Similar"}`}
                />
              )}
              {recommendations && recommendations.length > 0 && (
                <MovieList
                  ...
                  title={`${
                    language === "vi" ? "Gợi ý cho bạn" : "Recommendations"
                  }`}
                />
              )}
            </div>
      ...
      <MyModal isOpen={openModal} onRequestClose={closeModal}>
        {videosTrailer ? (
          videosTrailer.length > 0 ? (
            ...
            <div className="sm:w-lg md:w-xl lg:w-2xl text-2xl text-black">
              {language === "en-US"
                ? "No videos found for your country."
                : "Không tìm thấy video nào cho khu vực của bạn."}
            </div>
          )
        ) : (
          <></>
